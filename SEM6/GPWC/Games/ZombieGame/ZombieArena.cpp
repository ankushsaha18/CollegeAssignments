...
int main()
{
	...
	textureMenuBackground.loadFromFile("graphics/background.png");
	Sprite spriteMenuBackground;
	spriteMenuBackground.setTexture(textureMenuBackground);
	spriteMenuBackground.setScale(
		resolution.x / textureMenuBackground.getSize().x,
		resolution.y / textureMenuBackground.getSize().y);
	RectangleShape menuOverlay(resolution);
	menuOverlay.setFillColor(Color(0, 0, 0, 120));
	...
	// Text shown when the game is not actively playing
	Font font;
	...
	titleText.setString("Zombie Arena");
	...
	messageText.setPosition(80, 210);
	...
	while (window.isOpen())
	{
		...
		*/
		...
		if (state == State::LEVELING_UP)
		{
			...
			messageText.setString("Choose an upgrade:\n\n1 - Start next wave\n2 - Start next wave\n3 - Start next wave\n4 - Start next wave\n5 - Start next wave\n6 - Start next wave");
